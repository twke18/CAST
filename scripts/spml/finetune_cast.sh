...
# the pre-trained CAST that is fine-tuned on PASCAL
# VOC 2012. Users could also modify from this script
# for their use case.
#

# Set up parameters for network.
BACKBONE_TYPES=cast_small_p512_finetune
EMBEDDING_DIM=384

# Set up parameters for training.
PREDICTION_TYPES=softmax_classifier
TRAIN_SPLIT=train+
GPUS=0,1,2,3
LR_POLICY=poly
USE_SYNCBN=true
...
SEM_ANN_LOSS_TYPES=softmax # segsort / none
...
SNAPSHOT_DIR=${SNAPSHOT_ROOT}/finetune_test
echo ${SNAPSHOT_DIR}
...
PRETRAINED=${SNAPSHOT_ROOT}/checkpoint_0399.pth.tar
TRAIN_DATA_LIST=SPML/datasets/voc12/panoptic_${TRAIN_SPLIT}_hed.txt
...
# Build configuration file for training network.
...
# Train for the semantic segmentation.
if [ ${IS_TRAIN} -eq 1 ]; then
  python3 SPML/pyscripts/train/train_softmax.py\
    --data_dir ${DATAROOT}\
    --data_list ${TRAIN_DATA_LIST}\
    --snapshot_dir ${SNAPSHOT_DIR}\
    --cfg_path ${SNAPSHOT_DIR}/config.yaml

  python3 SPML/pyscripts/inference/inference_softmax_suppix.py\
    --data_dir ${DATAROOT}\
