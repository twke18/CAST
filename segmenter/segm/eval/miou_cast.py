...
def save_im(save_dir, save_name, im, seg_pred, seg_gt, colors, blend, normalization):
    ...
    seg_gt = seg_gt.data.cpu().numpy().astype(np.uint8)
    seg_pred = seg_pred.data.cpu().numpy().astype(np.uint8)
    for i in range(pred_rgb.shape[0]):
        if blend:
            blend_pred = blend_im(im_uint[i], seg_pred_uint[i])
            blend_gt = blend_im(im_uint[i], seg_rgb_uint[i])
            ims = (im_uint[i], blend_pred, blend_gt, seg_gt, seg_pred)
        else:
            ims = (im_uint[i], seg_pred_uint[i], seg_rgb_uint[i], seg_gt, seg_pred)
