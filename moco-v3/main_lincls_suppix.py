...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if 'cast' in args.arch:
        model = cast_models.cast.__dict__[args.arch]()
    elif 'tome' in args.arch:
        model = models.tome_suppix.__dict__[args.arch]()
    else:
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, segments, target) in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)

        if args.gpu is not None:
            images = images.cuda(args.gpu, non_blocking=True)
            segments = segments.cuda(args.gpu, non_blocking=True)
        if torch.cuda.is_available():
            target = target.cuda(args.gpu, non_blocking=True)

        # compute output
        output = model(images, segments)
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, segments, target) in enumerate(val_loader):
            if args.gpu is not None:
                images = images.cuda(args.gpu, non_blocking=True)
                segments = segments.cuda(args.gpu, non_blocking=True)
            if torch.cuda.is_available():
