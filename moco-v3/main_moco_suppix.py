...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if 'cast' in args.arch:
        ...
        model = suppix_utils.builder.MoCo_SupPix(
            partial(models.tome_suppix.__dict__[args.arch], stop_grad_conv1=args.stop_grad_conv1),
            args.moco_dim, args.moco_mlp_dim, args.moco_t)
    else:
...
def train(train_loader, model, optimizer, scaler, summary_writer, epoch, args):
    ...
    moco_m = args.moco_m
    for i, (images, segments, _) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            ...
            segments[0] = segments[0].cuda(args.gpu, non_blocking=True)
            segments[1] = segments[1].cuda(args.gpu, non_blocking=True)

        # compute output
        with torch.cuda.amp.autocast(True):
            loss = model(images[0], images[1], segments[0], segments[1], moco_m)
